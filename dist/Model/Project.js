...
var Project = /*#__PURE__*/function () {
  function Project(id, title, image, used_technologies, description, github_link) {
  ...
  _createClass(Project, [{
    ...
    value: function findAll(callback) {
      ...
      db.query(this.sqlQuery, function (err, result) {
        if (err) throw err;
        return callback(result.map(function (_ref) {
          var id = _ref.id,
              title = _ref.title,
              image = _ref.image,
          ...
          return new Project(id, title, image, used_technologies, description, github_link);
        }));
      });
    }
  }, {
    ...
    value: function findById(id, callback) {
      ...
      db.query(this.sqlQuery, [id], function (err, result) {
        if (err) throw err;
        return callback(result.find(function (_ref2) {
          var id = _ref2.id,
              title = _ref2.title,
              image = _ref2.image,
    ...
    value: function insert(_ref3, callback) {
      ...
      this.sqlQuery = "INSERT INTO project(title, image, used_technologies, description,github_link )VALUES(?,?,?,?,?) ";
      db.query(this.sqlQuery, [title, image, used_technologies, description, github_link], function (err, result) {
    ...
    value: function update(id, _ref4, callback) {
      ...
      this.sqlQuery = "UPDATE project SET title=?, image=?, used_technologies=?, description=?,github_link =? WHERE id = ?";
      db.query(this.sqlQuery, [title, image, used_technologies, description, github_link, id], function (err, result) {
